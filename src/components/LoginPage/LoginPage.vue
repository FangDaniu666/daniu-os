<template>
  <div class="tw-w-screen tw-h-screen main-background tw-select-none tw-overflow-hidden">
    <div ref="background_image"
         class="tw-absolute tw-w-full tw-h-full tw-flex tw-flex-col tw-justify-center tw-items-center"
         v-if="show_keyboard">
    ...
    <div ref="content" class="tw-flex tw-flex-col tw-w-full tw-h-full">
      <div ref="background_image"
           class="tw-absolute tw-w-full tw-h-full tw-flex tw-flex-row tw-justify-center tw-items-center">
        <div class="tw-w-140"></div>
        <img src="../../assets/images/deepin_3.png" alt="" class="tw-w-160" style="opacity:.03;min-width:600px"
             @load="bgloaded">
      </div>

      <div ref="header"
           class="tw-w-full tw-h-32 tw-flex-none tw-flex tw-flex-col tw-items-center tw-justify-center  tw-z-10">
        <div class="tw-text-4xl tw-tracking-wider tw-light tw-font-sans">
          {{ PrefixZero(date_hour, 2) }}:{{ PrefixZero(date_minute, 2) }}
        </div>
        <div class="tw-mt-1.5 tw-text-sm">{{ date_year }}/{{ date_month }}/{{ date_date }}
          {{ date_weekday_display }}
        </div>
      </div>

      <div ref="middle"
           class="tw-w-full tw-h-20 tw-flex-grow tw-flex  tw-flex-col tw-justify-center tw-items-center  tw-z-10"
           style="min-height:300px">

        <div class="tw-w-72 tw-h-40 tw-rounded-xl tw-flex tw-flex-col tw-flex-nowrap"
             style="background-color:rgb(241,204,247)" v-if="show_middle">

          <div class="tw-h-10 tw-flex  tw-justify-center tw-items-end ">
            <div class="tw-h-full " style="width:31%">
              <div class="ratio-container">
                <div class="ratio-content tw-rounded-lg  tw-bg-center tw-bg-cover"
                     :style="{ backgroundImage: 'url(' + require('@/assets/images/avatar.jpg') + ')' }">
                </div>
              </div>
            </div>
          </div>
          ...
          <div
              class="tw-h-full tw-flex tw-justify-center tw-items-center tw-text-2xl tw-tracking-wide tw-pt-4">
            {{ user_name }}
          </div>

          <div class=" tw-absolute tw-w-72 tw-h-40 " style="pointer-events:none;">
            <div ref="infinite_bar" class="tw-absolute tw-rounded-lg "
                 style="bottom:12px;height:38px;width:264px;left:12px"
                 :class="{'process-animation':relay,'tw-hidden':!show_loading_bar}"></div>
          </div>

          <div class="tw-h-full tw-flex tw-justify-center tw-items-center tw-py-3 tw-px-3">
            <input ref="password_input_bar" v-model="password" @keyup.enter="login_clicked" type="password"
                   placeholder="Enter your password" autofocus autocomplete
                   class="tw-w-full tw-h-full tw-rounded-lg"
                   style="background-color:rgb(243,214,248);outline-color: rgba(93,240,253,0.5);text-align: center">
          </div>
        </div>

        <button
            class="tw-rounded-full tw-w-12 tw-h-12 tw-mt-2 tw-bg-blue-600 tw-flex tw-items-center tw-justify-center hover:tw-bg-blue-700 tw-outline-none animate__animated"
            v-if="show_middle" @click="login_clicked" :class="{'animate__shakeX':button_shaking}">
          <v-icon class="tw-text-gray-100">mdi-arrow-right</v-icon>
        </button>


        <div v-if="!show_middle"
             class="tw-h-full tw-w-full  tw-flex tw-flex-row tw-flex-nowrap tw-justify-evenly tw-items-center"
             style="max-width:650px;min-width:300px" @click="blanker">
      ...
      <div ref="footer" class="tw-w-full tw-h-32 tw-flex-none tw-flex  tw-z-10 tw-flex-wrap">
        <div class="tw-w-76 tw-h-full flex-none tw-flex tw-justify-center tw-items-center"
          ...
          <div class="flex-none tw-flex tw-flex-col-reverse tw-pl-3">
            ...
            <div class="tw-absolute tw-text-gray-400">20.2 Community Edition</div>
          </div>
        </div>
        <div class="tw-h-full tw-flex-grow"></div>
        <div class="tw-h-full flex-none tw-w-76 tw-flex tw-justify-center tw-items-center tw-pr-2"
             style="min-width:300px">
          <button
              class="tw-rounded-full tw-w-12 tw-h-12 tw-bg-mygray-light hover:tw-bg-mygray-dark active:tw-bg-mygray-darker tw-flex tw-justify-center tw-items-center  tw-outline-none"
              @click="show_keyboard_clicked()">
            <v-icon class="tw-text-gray-100 ">mdi-keyboard</v-icon>
          </button>
          <button
              class="tw-rounded-full tw-w-12 tw-h-12 tw-bg-mygray-light hover:tw-bg-mygray-dark active:tw-bg-mygray-darker ml-8 tw-flex tw-justify-center tw-items-center tw-outline-none"
...
<style scoped>
.main-background {
  background-image: linear-gradient(to bottom right, rgb(117, 185, 234), rgb(239, 166, 249));
  color: #fdfafa
}
