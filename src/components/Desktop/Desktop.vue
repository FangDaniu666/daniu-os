  ...
  <div class=" tw-antialiased tw-w-screen tw-h-screen tw-overflow-hidden tw-flex tw-justify-center tw-items-center"
    ...
    <div class="tw-w-full tw-h-full tw-relative">
      <div ref="background" class="tw-relative tw-pb-9/16 tw-h-full tw-select-none">
        <img src="../../assets/images/desktop_1.jpg" alt="" class="tw-absolute tw-h-full tw-object-cover"
             @load="bgloaded"/>
      </div>
    </div>
    <div ref="frontground" class="tw-absolute tw-w-full tw-h-full tw-z-10 tw-overflow-hidden"
      ...
      <div v-for="item in window_list" :key="item.uuid">
        <WindowFolder v-if="item.type==='explorer'" :uuid='item.uuid' :startpos_x="item.spx"
                      :startpos_y="item.spy" :filemap="map" :openpath="item.openpath" :zindex="item.zindex"
        ...
        <WindowText v-if="item.type==='text'" :uuid='item.uuid' :startpos_x="item.spx" :startpos_y="item.spy"
                    ...
                    :minimized="item.minimized"/>
        <WindowBrowser v-if="item.type==='browser'" :uuid='item.uuid' :startpos_x="item.spx"
                       :startpos_y="item.spy" :zindex="item.zindex" :default_width="item.default_width"
                       :minimized="item.minimized"/>
        <WindowVSCode v-if="item.type==='vscode'" :uuid='item.uuid' :startpos_x="item.spx"
                      :startpos_y="item.spy" :zindex="item.zindex" :minimized="item.minimized"/>
        <WindowMusic v-if="item.type==='music'" :uuid='item.uuid' :startpos_x="item.spx" :startpos_y="item.spy"
                     :zindex="item.zindex" :default_width="item.default_width"
                     :default_height="item.default_height" :fixedsize="item.fixedsize"
                     :minimized="item.minimized"/>
        <WindowSetting v-if="item.type==='settings'" :uuid='item.uuid' :startpos_x="item.spx"
                       :startpos_y="item.spy" :zindex="item.zindex" :minimized="item.minimized"/>
        <WindowTerminal v-if="item.type==='terminal'" :uuid='item.uuid' :startpos_x="item.spx"
                        :startpos_y="item.spy" :zindex="item.zindex" :minimized="item.minimized"/>
      </div>
    </div>
    <div ref="realbackground" class="tw-absolute tw-w-full tw-h-full realbackground" style="top:0;left:0"
      ...
      <div class="tw-absolute tw-w-full " @click="background_clicked"
        ...
        <div ref="keyboard_div" class="tw-absolute "
             style="z-index:1001;width:600px;height:200px;top:0px;left:0px" v-if="desktop_keyboard_show">
...
export default {
  ...
  created() {
    ...
    let config = {
      ...
      url: this.$store.state.base_url + '/maps/selectAll',
      headers: {}
    };

    axios.request(config)
        .then((res) => {
          this.map_add_uuid(res.data.data)
          this.map = res.data.data
          console.log("error:", this.map)

          this.$store.commit('commit_filemap', res.data.data)
          this.deal_with_postfilename()
        })
        .catch((err) => {
          console.log("error:", err)
