<template>
    <Window @height_changed="window_height_changed" :startpos_x="startpos_x" :startpos_y="startpos_y" :uuid="uuid"
            :zindex="zindex" :minimized="minimized" @width_changed="window_width_changed" v-show="loaded">
        <template v-slot:header>
            <div class="tw-flex tw-items-center tw-select-none" style="pointer-events:none;">
                <img src="../../assets/images/icons/doc.png" alt="" style="pointer-events:auto;"
                     class=" tw-w-8 tw-h-7 tw-ml-4">
                <div
                    class="tw-h-9 tw-flex tw-items-center tw-justify-center tw-rounded-lg tw-bg-mygray-b4 hover:tw-bg-mygray-b5 tw-ml-3"
                    style="pointer-events:auto;" :style="{'opacity':global_focus===uuid?'1':'0.85'}"
                    @click="title_clicked">
                ...
                <div
                    class="tw-h-9 tw-px-3  tw-flex tw-items-center tw-justify-center tw-bg-mygray-b3  tw-rounded-lg tw-ml-3 hover:tw-bg-mygray-b1 tw-text-mygray-b8"
        ...
        <template v-slot:content>
            <div class="tw-w-full tw-h-full tw-rounded-b-2xl tw-flex tw-flex-col-reverse"
                 @contextmenu.prevent="mr_clicked">
                <div
                    class="tw-h-8 tw-w-full tw-rounded-b-2xl tw-border-t tw-border-gray-200 tw-items-center tw-flex tw-flex-row-reverse"
                    style="background-color:#fdfdfd">
                    <div
                        class="tw-flex-none tw-h-full tw-flex tw-justify-center tw-items-center tw-px-3 tw-py-1 tw-select-none ">
                        <button
                            class="tw-flex tw-justify-center tw-items-center hover:tw-bg-mygray-b1 tw-px-2 rounded-lg tw-text-xs tw-text-gray-700 tw-w-full tw-h-full tw-outline-none">
                            None
                    ...
                    <div
                        class="tw-flex-none tw-h-full tw-flex tw-justify-center tw-items-center tw-px-3 tw-py-1 tw-select-none">
                        <div
                            class="tw-flex tw-justify-center tw-items-center rounded-lg tw-text-xs tw-text-gray-700 tw-w-full tw-h-full">
                            Browse
                        </div>
                    </div>
                    <div
                        class="tw-flex-none tw-h-full tw-flex tw-justify-center tw-items-center tw-pr-24 tw-py-1 tw-select-none">
                        <div
                            class="tw-flex tw-justify-center tw-items-center rounded-lg tw-text-xs tw-text-gray-700 tw-w-full tw-h-full">
                    ...
                    <div
                        class="tw-flex-grow tw-h-full tw-flex tw-flex-row tw-justify-start tw-py-1 tw-select-none tw-pl-4">
                        <div
                            class="tw-flex tw-justify-start tw-items-center rounded-lg tw-text-xs tw-text-gray-700 tw-w-18 tw-h-full tw-flex-nowrap">
                ...
                <div class=" tw-flex-grow tw-w-full tw-flex flex-row">
                    ...
                    <div class="tw-w-full  tw-bg-white tw-pl-5 tw-pr-3 tw-py-1.5 change-srollbar" style="overflow:auto"
                         :style="{'height': cont_height +'px'}">
                        <markdown-it-vue class="md-body zoomined-frame" :content="content"
                                         :style="{'width':cont_width+'px !important'}"
    ...
    mounted() {
        ...
        /*this.$axios.raw(this.filesrc)
            .then((res) => {
                ...
                console.log("Formatted JSON data:", JSON.stringify(res, null, 2)); // 2代表缩进空格数
                res.data.data = Buffer.from(res.data.data, 'base64').toString('utf8')
        ...
        let config = {
            ...
            url: this.$store.state.base_url + '/docs/selectOne?name=' + this.filesrc,
            headers: {}
        };
        ...
        axios.request(config)
            .then((res) => {
                ...
                console.log("Got data:", this.filesrc)
